...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.model_class_name is not None:
        ...
        # 优先检查 config，支持自定义 checkpoint 路径不含模型名的情况
        _cfg_check = AutoConfig.from_pretrained(model_args.model_name_or_path, trust_remote_code=True)
        _arch = getattr(_cfg_check, "architectures", []) or []
        _is_llava_qwen = (
            getattr(_cfg_check, "model_type", "") in ("llava_qwen_flash", "llava_qwen")
            or any("LlavaQwen" in a for a in _arch)
        )
        if _is_llava_qwen:
            from llava.model.language_model.llava_qwen_flash import LlavaQwenForCausalLM_Flash
            model = LlavaQwenForCausalLM_Flash.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                attn_implementation=training_args.attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
...
def train(attn_implementation=None):
    ...
    # 检测是否是 LlavaQwen 系列（路径不含 qwen 时通过 config 判断）
    _tokenizer_cfg = AutoConfig.from_pretrained(model_args.model_name_or_path, trust_remote_code=True)
    _is_qwen_model = (
        "qwen" in model_args.model_name_or_path.lower()
        or getattr(_tokenizer_cfg, "model_type", "") in ("llava_qwen_flash", "llava_qwen", "qwen2")
        or any("LlavaQwen" in a for a in (getattr(_tokenizer_cfg, "architectures", []) or []))
    )
