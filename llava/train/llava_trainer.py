...
try:
    ...
    # transformers >= 4.50 removed ALL_LAYERNORM_LAYERS
...
class LLaVATrainer(Trainer):
    ...
    def create_accelerator_and_postprocess(self):
        ...
        # dispatch_batches/split_batches 在新版 transformers 中移至 accelerator_config
        _acc_kwargs = dict(
        ...
        if hasattr(self.args, "dispatch_batches"):
            _acc_kwargs["dispatch_batches"] = self.args.dispatch_batches
        if hasattr(self.args, "split_batches"):
            _acc_kwargs["split_batches"] = self.args.split_batches
        self.accelerator = Accelerator(**_acc_kwargs)
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if not isinstance(train_dataset, torch.utils.data.IterableDataset):
            ...
            _sw_sig = inspect.signature(seed_worker)
            if len(_sw_sig.parameters) > 1:
                # transformers >= 4.50: seed_worker(worker_id, num_workers, rank)
